...
const SPREADSHEET_ID = '1jkljhiNHAlHTIMieh9FQKa983dBwrVh_a0DISd57mn0';
...
const CERTIFICATE_FOLDER_ID = '1hxqkPy3WR3bB_GvLhhOTqnT_EwzFuo3C'; // ใส่ ID ของโฟลเดอร์ certificates

// ==========================================
// Function สำหรับรับ GET Request (ขอ Certificate URL)
// ==========================================
function doGet(e) {
  try {
    ...
    if (action === 'getCertificate') {
      ...
      if (!email) {
        return ContentService
          .createTextOutput(JSON.stringify({error: 'Email required'}))
          .setMimeType(ContentService.MimeType.JSON);
      }
      
      console.log('Getting certificate for:', email);
      
      // ค้นหา Certificate ในโฟลเดอร์
  ...
  try {
    ...
    const response = {
      ...
      'data_received': {
        ...
        'certificate_file_id': certificateFileId,  // เพิ่ม File ID
        'certificate_url': certificateUrl,
        'certificate_download_url': certificateDownloadUrl,  // เพิ่ม Direct Download URL
        'email_sent': true
      }
    };
...
function testEmailSending() {
  ...
  try {
    ...
    const testCertUrl = 'https://drive.google.com/file/d/test123/view';
